...
```{r}
morph_data %>% mutate(date_diff1 = as.numeric(as.Date(morph_data$price_date, "%m/%d/%y")-as.Date("2013-01-01")))
```
...
```{r}
## maybe consider only incorporate grouping variable at this time? But the results are the same
mod_11 <- lmer(data=morph_data, log(ppm) ~ (1 |state), REML=F)
mod_12 <- lmer(data=morph_data, log(ppm) ~ (1 |city), REML=F)
mod_13 <- lmer(data=morph_data, log(ppm) ~ (1 |usa_region), REML=F)

BIC_score1 <- sapply(c(mod_11, mod_12, mod_13), BIC)
re_results1 <- data.frame('Grouping' = c('State', 'City', 'Region'), 'BIC' = BIC_score1)
kable(re_results1)
```
...
mod_drop_quater <- lmer(data=morph_data, log(ppm) ~ (1 |state) + date_diff  + mgstr +
                bulk_purchase + primary_reason + source, REML=F)

mod_drop_bulk_purchase <- lmer(data=morph_data, log(ppm) ~ (1 |state) + date_diff + quarter + mgstr 
                               + primary_reason + source, REML=F)

mod_drop_primary_reason <- lmer(data=morph_data, log(ppm) ~ (1 |state) + date_diff + quarter + mgstr +
...
mod_drop_source <- lmer(data=morph_data, log(ppm) ~ (1 |state) + date_diff + quarter + mgstr +
                bulk_purchase + primary_reason, REML=F)


anova(mod_1,mod_drop_date)$`Pr(>Chisq)`[2] 
anova(mod_1,mod_drop_quater)$`Pr(>Chisq)`[2] # add
...
We now have `quarter`, `bulk_purchase`, `primary_reason` and `mgstr` in our model, regarding `state` as the grouping variable.


# ```{r}
# full_model <- lmer(data=morph_data, log(ppm) ~ (1 |state) + date_diff + as.factor(quarter) + mgstr +
#                      bulk_purchase + as.factor(primary_reason) + as.factor(source), REML=F)
# ```
# 
# ```{r}
# # backward elimination
# step_res <- step(full_model)
...
**log(ppm) ~  bulk_purchase + quarter + primary_reason + mgstr + (1 | state)**

```{r}
mod_final <- lmer(data=morph_data, log(ppm) ~ (1 |state)   + quarter  + mgstr +
                bulk_purchase + primary_reason, REML=F)
